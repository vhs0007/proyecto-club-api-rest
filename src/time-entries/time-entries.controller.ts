...
export class TimeEntriesController {
  ...
  @Post()
  create(@Body() createTimeEntryDto: CreateTimeEntryDto): Promise<TimeEntryResponseDto> {
    return this.timeEntriesService.create(createTimeEntryDto);
  }

  @Get()
  findAll(@Query('clubId', ParseIntPipe) clubId: number): Promise<TimeEntryResponseDto[]> {
  ...
  @Patch(':id')
  update(@Param('id') id: string, @Body() updateTimeEntryDto: UpdateTimeEntryDto): Promise<TimeEntryResponseDto> {
