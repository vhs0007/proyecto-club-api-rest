...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto): Promise<UserResponseDto> {
    const existingByDocument = await this.usersRepository.findByDocument(
    ...
    const typeExists = await this.usersRepository.existsTypeId(
      createUserDto.typeId,
    );
    ...
    if (createUserDto.typeId === UserType.ATHLETE) {
      if (createUserDto.gender != null && createUserDto.gender.trim() !== '') {
        ...
        if (g !== 'masculino' && g !== 'femenino')
          throw new BadRequestException('gender must be masculino or femenino');
      }

      if (createUserDto.birthDate != null) {
        if (new Date(createUserDto.birthDate) > new Date())
          throw new BadRequestException('birthDate cannot be in the future');
      }
    }

    if (
      createUserDto.typeId === UserType.WORKER &&
      createUserDto.startWorkAt != null &&
      createUserDto.endWorkAt != null
    ) {
      if (
        new Date(createUserDto.startWorkAt) >= new Date(createUserDto.endWorkAt)
      )
