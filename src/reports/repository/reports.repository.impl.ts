...
export class ReportsRepository implements IReportsRepository {
  ...
  async getSalariesReport(
    ...
    return {
      ...
      hoursToWorkPerMonth: user.hoursToWorkPerDay
        ? user.hoursToWorkPerDay * 30
        : 0,
      extraHours: 0,
  ...
  async getMonthIncomeReport(
    ...
    const memberships: MembershipFromPrisma[] =
    ...
    const activities: ActivityFromPrisma[] =
    ...
    const monthIncomeMemberships = memberships.reduce(
      (acc, membership) => acc + membership.type.price.toNumber(),
      0,
    );
    const monthIncomeActivities = activities.reduce(
      (acc, activity) => acc + activity.cost.toNumber(),
      0,
    );
    return {
      month: monthStart,
  ...
  async getMonthlyProgressionIncomeReport(
    ...
    const totalIncome = monthlyIncomes.reduce(
      (acc, month) => acc + month.monthIncomeTotal,
      0,
    );
    const totalIncomeMemberships = monthlyIncomes.reduce(
      (acc, month) => acc + month.monthIncomeMemberships,
      0,
    );
    const totalIncomeActivities = monthlyIncomes.reduce(
      (acc, month) => acc + month.monthIncomeActivities,
      0,
    );
    return {
      dateStart: start,
